...
<section class={`flex flex-col ${showMenu === true ? "gap-3" : "gap-0"}`} style="transition:all 1000ms cubic-bezier(0.83, 0, 0.17, 1);">
	...
	>
		...
		<form method="post" class="m-0 w-full flex flex-col gap-2" use:enhance>
			<input class="w-full p-2 rounded-md border border-gray-500/30 bg-gray-950/40" type="text" name="id" placeholder="ID" />
			<input class="w-full p-2 rounded-md border border-gray-500/30 bg-gray-950/40" type="number" name="strengthToUSD" placeholder="1.0 Strength -> USD" />
			<input class="w-full p-2 rounded-md border border-gray-500/30 bg-gray-950/40" type="text" name="algorithm" placeholder="Algorithm" />
	...
	<div class="w-[350px] border h-full border-gray-500/40 bg-gray-500/20 rounded-md p-3 flex flex-col gap-2 relative">
		<div class="flex gap-2">
			...
			<button
				...
				class="w-1/4 text-2xl border-green-800 bg-gray-950/70 hover:bg-green-500 hover:text-black transition-all">+</button
			>
		</div>
		{#each accounts as { address, balance, status, id }}
			<button
			...
			>
				...
				<p class="balance text-left text-ellipsis overflow-hidden whitespace-nowrap m-0">Net Worth: {Number.parseFloat(balance).toFixed(10)} USD</p>
			</button>
